...
async def test_rate_post_positive(client: AsyncClient, test_user, auth_headers):
    ...
    # Rate the post
    response = await client.post(f"/posts/{post_id}/rate", headers=auth_headers, json={"value": 1})
...
async def test_remove_rating(client: AsyncClient, test_user, auth_headers):
    ...
    # Rate the post first
    await client.post(f"/posts/{post_id}/rate", headers=auth_headers, json={"value": 1})

    # Remove rating
    response = await client.post(f"/posts/{post_id}/rate", headers=auth_headers, json={"value": 0})
