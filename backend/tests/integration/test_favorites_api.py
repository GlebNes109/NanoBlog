...
async def test_add_to_favorites(client: AsyncClient, test_user, auth_headers):
    ...
    # Add to favorites
    response = await client.post(f"/favorites/{post_id}", headers=auth_headers)
...
async def test_get_favorites(client: AsyncClient, test_user, auth_headers):
    # Create a post first
    post_response = await client.post(
        "/posts", headers=auth_headers, json={"title": "Favorite Post", "content": "Content"}
    )
    post_id = post_response.json()["id"]

    # Add to favorites
    await client.post(f"/favorites/{post_id}", headers=auth_headers)
...
async def test_remove_from_favorites(client: AsyncClient, test_user, auth_headers):
    ...
    response = await client.delete(f"/favorites/{post_id}", headers=auth_headers)
