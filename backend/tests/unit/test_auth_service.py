...
async def test_login_success(mock_user_repository, mock_password_hasher, sample_user_read):
    ...
    with patch('src.application.auth_service.create_access_token') as mock_token:
        ...
        mock_password_hasher.verify.assert_called_once_with("password123", sample_user_read.password)
...
@pytest.mark.asyncio
async def test_login_incorrect_password(mock_user_repository, mock_password_hasher, sample_user_read):
